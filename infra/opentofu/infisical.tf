module "infisical" {
  ...
  count  = local.infisical_lxc_enabled ? 1 : 0
...
module "infisical_vm" {
  ...
  count  = local.infisical_enabled && local.infisical_runtime_type == "vm" ? 1 : 0

  description   = var.infisical_container_description
  node_name     = var.proxmox_node_name
  vm_id         = var.infisical_container_vmid
  name          = var.infisical_container_hostname
  started       = var.infisical_started
  start_on_boot = var.infisical_start_on_boot
  tags          = ["infisical", "secrets", "opentofu"]
  ...
  cloud_init_user = coalesce(try(local.infisical_runtime.cloud_init_user, null), var.guest_vm_cloud_init_user)
  ssh_public_keys = var.lxc_ssh_public_keys
