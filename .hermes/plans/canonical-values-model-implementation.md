...
- [ ] Render `ansible-vars.json` from services with non-secret configuration and placement.
...
- [~] Record `schema_version`, loader/renderer version, source commit, selected site, `model_digest`, secret identity, ciphertext hash, projection digest, enabled services, ownership, and tool versions in plan metadata. Projection manifest identity exists; plan metadata integration remains.
