...
| 2026-07-28 | Report-only normalized plan-equivalence helper | `docker compose run --rm -T infra python -m unittest -v tests/test_plan_equivalence.py tests/test_tfplan_metadata.py`; `docker compose run --rm -T infra python -m unittest discover -s tests -p 'test_*.py'`; `docker compose run --rm -T infra python -m py_compile scripts/plan_equivalence.py tests/test_plan_equivalence.py`; `git diff --check` | `[x]` | 22 focused equivalence/metadata tests and 336 full repository tests passed; normalized public fixtures compare exact resource addresses, actions, and values, ignore only declared formatting/path metadata, redact differences to address/kind, and remain unwired to plan/apply. |
