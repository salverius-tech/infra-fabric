...
| 2026-07-28 | Canonical Ansible projection contract | `docker compose run --rm -T infra python -m unittest -v tests/test_ansible_canonical_compat.py tests/test_canonical_values.py tests/test_projection_manifest.py`; `docker compose run --rm -T infra python -m unittest discover -s tests -p 'test_*.py'`; `docker compose run --rm -T infra python -m py_compile tests/test_ansible_canonical_compat.py`; `git diff --check` | `[x]` | 32 focused projection/catalog tests and 345 full repository tests passed; enabled catalog services have consistent canonical host/group ownership, disabled services are omitted, service vars preserve resource/endpoints/release ownership, and no sensitive sentinel is emitted. |
