...
| 2026-07-28 | Atomic canonical projection replacement | `docker compose run --rm -T infra python -m unittest -v tests/test_plan_projection_lifecycle.py tests/test_canonical_values.py tests/test_projection_manifest.py`; `docker compose run --rm -T infra python -m unittest discover -s tests -p 'test_*.py'`; `bash -n scripts/plan-infra.sh`; `git diff --check` | `[x]` | 31 focused lifecycle/projection tests and 331 full repository tests passed; plan refresh preserves the prior generated projection set until the new set installs, restores it if installation fails, and removes the prior set only after successful replacement. |
