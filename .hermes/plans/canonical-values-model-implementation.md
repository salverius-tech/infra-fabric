...
| 2026-07-28 | Dry-run migration backup manifest artifact | `docker compose run --rm -T infra python -m unittest -v tests/test_migration_backup.py`; `docker compose run --rm -T infra python -m unittest discover -s tests -p 'test_*.py'`; `docker compose run --rm -T infra python -m py_compile scripts/migration_backup.py tests/test_migration_backup.py`; `git diff --check` | `[x]` | 8 focused migration-backup tests and 348 full repository tests passed; explicit selected regular files can be staged into a disposable tree and emitted as a new mode-0600 content-free manifest, while source files and existing outputs remain untouched. |
