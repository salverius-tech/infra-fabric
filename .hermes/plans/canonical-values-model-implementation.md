...
| 2026-07-28 | Metadata-only SOPS availability slice | `docker compose run --rm -T infra python -m unittest -v tests/test_secret_provider.py`; `docker compose run --rm -T infra python -m unittest discover -s tests -p 'test_*.py'`; `docker compose run --rm -T infra python -m py_compile scripts/secret_provider.py tests/test_secret_provider.py`; `git diff --check` | `[x]` | 15 focused secret-provider tests and 309 full repository tests passed; availability checks validate bundle/executable/key-file prerequisites and optional recipient metadata without invoking decryption or exposing secret/private-key material. Policy wiring remains deferred. |
