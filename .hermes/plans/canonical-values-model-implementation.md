...
| 2026-07-28 | Migration backup manifest rehearsal | `docker compose run --rm -T infra python -m unittest -v tests/test_migration_backup.py tests/test_migrate_site_values.py tests/test_projection_manifest.py`; `docker compose run --rm -T infra python -m unittest discover -s tests -p 'test_*.py'`; `docker compose run --rm -T infra python -m py_compile scripts/migration_backup.py tests/test_migration_backup.py`; `git diff --check` | `[x]` | 17 focused migration/manifest tests and 342 full repository tests passed; deterministic content-free manifests hash regular files, reject traversal/symlink/duplicate/unexpected/changed paths, and pass a disposable restore rehearsal without wiring into migration mutation. |
