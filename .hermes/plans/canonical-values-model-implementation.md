...
| 2026-07-28 | Strict logical-secret paths and backup symlink rejection | `docker compose run --rm -T infra python -m unittest -v tests/test_service_catalog.py tests/test_secret_provider.py tests/test_migration_backup.py`; `docker compose run --rm -T infra python -m unittest discover -s tests -p 'test_*.py'`; `docker compose run --rm -T infra python -m py_compile scripts/service_catalog.py scripts/secret_provider.py scripts/migration_backup.py`; `git diff --check` | `[x]` | 30 focused catalog/secret/backup tests and 348 full repository tests passed; catalog/provider logical paths require lowercase bounded identifiers, errors remain sanitized, and backup verification rejects unexpected symlinks instead of silently filtering them. |
