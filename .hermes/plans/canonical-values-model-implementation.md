...
- [~] Define external age-key discovery, missing-key errors, recipient checks, and provider availability checks. External key-file discovery, sanitized missing/unreadable/permission failures, and public SOPS age-recipient matching exist; provider availability policy remains.
...
- [ ] Define normalization for HCL quoting, CIDR/bare addresses, `null`, `dhcp`, booleans, lists, checksums, hostnames, generated names, and derived DNS records.
...
- [ ] Exclude Ansible-only, runtime-only, and unnecessary secret values.
- [ ] Update every OpenTofu invocation path to consume exactly the generated projection.
...
- [ ] Verify site/model/secret/projection identity before OpenTofu and again before Ansible.
...
| 2026-07-28 | Canonical plan identity slice | `docker compose run --rm -T infra python -m unittest -v tests/test_tfplan_metadata.py tests/test_values_context.py tests/test_projection_manifest.py`; `docker compose run --rm -T infra python -m unittest discover -s tests -p 'test_*.py'`; `docker compose run --rm -T infra python -m py_compile scripts/tfplan-metadata.py tests/test_tfplan_metadata.py`; `git diff --check` | `[x]` | 31 focused identity/context/manifest tests and 305 full repository tests passed; canonical metadata records verified site/model/projection/renderer/source identity, altered projections fail before apply verification, missing manifests fail creation, and legacy sites retain explicit `canonical: null`. |
