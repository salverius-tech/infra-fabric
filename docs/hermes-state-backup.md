...
```text
values/sites/<site>/service-backups/hermes/hermes-state-<timestamp>.tar.gz
values/sites/<site>/service-backups/hermes/hermes-state-<timestamp>.tar.gz.sha256
```
...
```bash
VALUES_SITE=<site> scripts/hermes-state.sh restore values/sites/<site>/service-backups/hermes/hermes-state-<timestamp>.tar.gz
```
...
Hermes state into `values/sites/<site>/service-backups/hermes/`, restores the selected
