...
Use it only to inspect a reviewed legacy root layout during recovery or migration analysis. The recipe accepts no arguments and executes one fixed report command in a Docker Compose service with networking disabled, a read-only root filesystem, and the repository mounted read-only. Python bytecode/cache output is disabled or redirected to temporary storage. It emits a value-redacted JSON report to stdout and does not rewrite, normalize, move, delete, generate, or resolve tagged images through a registry. It deliberately bypasses canonical site preflight because that preflight would make a legacy-only recovery source unreachable. It is not invoked by setup, validation, planning, apply, teardown, or Ansible execution; select the recovered canonical site and return to `just validate` after the bounded investigation.

The direct discovery CLI retains a separate `--resolve-tagged-images` opt-in for an explicitly authorized migration investigation. That direct option can contact a container registry and is intentionally unreachable through the forensic recipe/wrapper. Separately retained recovery CLIs remain subject to their own explicit authorization and safety contracts.
