...
Immediately before a saved plan is applied, the wrapper re-verifies plan metadata and creates a checksum-bound state snapshot when local state already exists. Snapshot files and manifests are mode `0600`, installation is atomic, and the newest ten snapshots are retained. The canonical apply path also creates a read-only execution snapshot containing the reviewed plan and metadata, canonical site model, generated projections, encrypted secret bundle, and exact-site SOPS policy. Storage preparation, provider credential resolution, OpenTofu apply, and Ansible orchestration consume that snapshot rather than mutable live inputs; the newest five execution snapshots are retained as private evidence. Generated projections, execution snapshots, and state snapshots default to separate mode-`0700` controller-local roots under `${XDG_STATE_HOME:-$HOME/.local/state}/infra-fabric/sites/<site>/`; generated projections are atomically published into the generated root's ordinary `generated/` child. Explicit absolute `INFRA_GENERATED_ROOT`, `INFRA_EXECUTION_SNAPSHOT_ROOT`, and `INFRA_STATE_SNAPSHOT_ROOT` overrides remain supported. The wrapper traverses these roots without following symlinks and mounts each only at its fixed tooling-container path; publication remains atomic/no-replace and fails closed if a configured path is unsafe or unsupported.
...
```text
python3 scripts/state-snapshot.py verify --snapshot <state-snapshot-root>/<snapshot>
python3 scripts/state-snapshot.py restore --snapshot <state-snapshot-root>/<snapshot> --state values/sites/<site>/terraform.tfstate --replace-existing
```
