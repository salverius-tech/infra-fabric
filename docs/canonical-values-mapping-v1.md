...
| `services.<name>.endpoints.public_names` | list of hostnames | `<service>_server_name`, `<service>_domain`, DNS JSON | Ansible/Caddy/DNS projection | canonical/derived | lowercase, strip trailing dot | normalized conflict fails | public | DNS/proxy |
