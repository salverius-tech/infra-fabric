...
The canonical inputs are `values/sites/<site>/site.yaml` and `values/sites/<site>/secrets.sops.yaml`; `generated/` is derived. Legacy migration and import tooling is retired. Never use a legacy `.env`, raw tfvars, direct `site.yml`, or raw OpenTofu command as a fallback authoring or recovery path. Resolve canonical-input problems with the validation and projection lifecycle in [canonical model operations](canonical-model-operations.md).
