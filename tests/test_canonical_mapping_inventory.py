...
class CanonicalMappingInventoryTests(unittest.TestCase):
    def test_current_sources_are_fully_classified(self) -> None:
        ...
        self.assertEqual(report["opentofu"]["variable_count"], 204)
        ...
        self.assertEqual(report["matrix_path_coverage"]["valid_count"], 250)
        ...
        self.assertEqual(report["matrix_classification_coverage"]["checked_count"], 310)
        self.assertEqual(report["matrix_classification_coverage"]["valid_count"], 310)
        ...
        self.assertEqual(report["consumer_evidence"]["token_count"], 254)
        self.assertEqual(report["consumer_evidence"]["exact_evidence_count"], 206)
        ...
        self.assertEqual(report["consumer_evidence"]["row_count"], 248)
        self.assertEqual(report["consumer_evidence"]["evidenced_row_count"], 248)
        ...
        self.assertTrue(
            all(
                reference["file"] and reference["lines"]
                for reference in first_evidence["exact"][0]["references"]
            )
        )
        self.assertEqual(report["source_reconciliation"]["source_identity_count"], 373)
        self.assertEqual(
            report["source_reconciliation"]["accounted_identity_count"], 373
        )
        ...
        self.assertEqual(
            report["source_inventory"]["coverage"]["opentofu_variables_inventoried"],
            204,
        )
        self.assertGreater(
            report["source_inventory"]["scaffold"]["terraform_assignment_count"], 0
        )
        self.assertIn(
            "a_records", report["source_inventory"]["scaffold"]["dns"]["top_level_keys"]
        )
        self.assertIn(
            "tf_vmid", report["source_inventory"]["ansible"]["inventory_fields"]
        )
        self.assertEqual(report["source_inputs"]["input_count"], 373)
        self.assertEqual(report["source_inputs"]["unique_identities"], 373)
        self.assertEqual(
        ...
        self.assertEqual(
            report["source_inputs"]["status"],
            "classification-complete-with-review-dispositions",
        )
        self.assertEqual(report["mapping_matrix"]["row_count"], 310)
        self.assertEqual(
            report["mapping_matrix"]["status"], "semantic-coverage-complete"
        )
        self.assertEqual(
            report["mapping_contract_evidence"],
            {
                "scope": "tracked-public-repository-producers-and-consumers",
                "source_producer_consumer_status": "complete",
                "provider_plan_equivalence": "not-claimed",
                "live_infrastructure_equivalence": "not-claimed",
            },
        )
        self.assertEqual(
            report["retired_alias_contract"],
            {
                "aliases": ["forgejo_runtime", "tailscale_client_enabled"],
                "opentofu_boundary": "explicit-null-preconditions",
                "scaffold_inputs": "absent",
                "status": "complete",
            },
        )
        self.assertEqual(report["matrix_coverage"]["input_count"], 360)
        self.assertEqual(report["matrix_coverage"]["source_input_count"], 373)
        self.assertEqual(report["matrix_coverage"]["excluded_count"], 13)
        self.assertEqual(
            report["matrix_coverage"]["matched_count"]
            + report["matrix_coverage"]["unmatched_count"],
            360,
        )
        self.assertEqual(report["matrix_coverage"]["matched_count"], 360)
        ...
        self.assertEqual(
            {
                (item["source"], item["key"], item["classification"])
        ...
        self.assertIn(
            {
                ...
                "key": "onramp_host_datastore_id",
                "canonical_path": "resources.shared_hosts.onramp_host.storage.root.storage_id",
            },
            report["matrix_coverage"]["matched"],
        )
        self.assertIn(
            {
                "source": "scripts/parse-env.py",
                "key": "PROXMOX_VE_ENDPOINT",
                "canonical_path": "platform.proxmox.endpoint",
            },
            report["matrix_coverage"]["matched"],
        )
        self.assertEqual(
        ...
        for source, key, canonical_path in (
            (
                ...
                "platform.lxc_template_download_timeout_seconds",
            ),
            (
                "scaffold/terraform.tfvars",
                "guest_vm_cloud_init_user",
                "platform.vm_cloud_init_user",
            ),
            (
                "scaffold/terraform.tfvars",
            ...
            (
                ...
                "services.forgejo.configuration.enable_caddy",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
                "forgejo_configure_system_ssh",
                "services.forgejo.configuration.configure_system_ssh",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
                "forgejo_write_initial_config",
                "services.forgejo.configuration.write_initial_config",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
                "forgejo_bootstrap_enabled",
                "services.forgejo.configuration.bootstrap_enabled",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
                "forgejo_bootstrap_admin_username",
                "services.forgejo.configuration.bootstrap_admin_username",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
            ...
            (
                ...
                "services.forgejo.configuration.bootstrap_owner_email",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
                "forgejo_actions_enabled",
                "services.forgejo.configuration.actions_enabled",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
                "forgejo_actions_default_url",
                "services.forgejo.configuration.actions_default_url",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
                "infisical_data_dir",
                "services.infisical.configuration.data_dir",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
                "infisical_postgres_user",
                "services.infisical.configuration.postgres_user",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
                "infisical_postgres_db",
                "services.infisical.configuration.postgres_db",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
            ...
            (
                "scaffold/ansible/inventory/local.yml",
                "forgejo_runner_url",
                "services.forgejo_runner.configuration.url",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
                "forgejo_runner_name",
                "services.forgejo_runner.configuration.name",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
                "forgejo_runner_scope",
                "services.forgejo_runner.configuration.scope",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
                "forgejo_runner_label",
                "services.forgejo_runner.configuration.label",
            ),
            (
                "scaffold/ansible/inventory/local.yml",
            ...
            (
                ...
                "services.searxng_onramp.configuration.container_port",
            ),
            (
                "scaffold/terraform.tfvars",
                "searxng_bind_address",
                "services.searxng_onramp.configuration.bind_address",
            ),
            (
                "scaffold/terraform.tfvars",
                "searxng_instance_name",
                "services.searxng_onramp.configuration.instance_name",
            ),
            (
                "scaffold/terraform.tfvars",
                "searxng_enable_public_url",
                "services.searxng_onramp.configuration.enable_public_url",
            ),
            (
        ...
        for key, canonical_path in (
            ...
            (
                "service_storage.forgejo.data.type",
                "resources.guests.forgejo.storage.volumes.data.type",
            ),
            (
                "service_storage.forgejo.data.storage_id",
                "resources.guests.forgejo.storage.volumes.data.storage_id",
            ),
            (
                "service_storage.forgejo.data.size_gb",
                "resources.guests.forgejo.storage.volumes.data.size_gb",
            ),
            (
                "service_storage.forgejo.data.target",
                "resources.guests.forgejo.storage.volumes.data.target",
            ),
            (
                "service_storage.forgejo.data.backup",
                "resources.guests.forgejo.storage.volumes.data.backup",
            ),
            (
                "service_storage.forgejo.data.read_only",
                "resources.guests.forgejo.storage.volumes.data.read_only",
            ),
        ):
            self.assertIn(
                {
        ...
        self.assertIn(
            {
                "source": "scaffold/terraform.tfvars",
                "key": "forgejo_runner_mac_address",
                "canonical_path": "resources.guests.forgejo_runner.network.mac_address",
            },
            report["matrix_coverage"]["matched"],
        )
        self.assertIn(
            {
                ...
                "key": "technitium_container_vmid",
                "canonical_path": "resources.guests.technitium.identity.vmid",
            },
            report["matrix_coverage"]["matched"],
        )
        self.assertIn(
            {
                "source": "scaffold/terraform.tfvars",
                "key": "forgejo_container_vmid",
                "canonical_path": "resources.guests.forgejo.identity.vmid",
            },
            report["matrix_coverage"]["matched"],
        )
        self.assertIn(
            {
                "source": "scaffold/terraform.tfvars",
                "key": "infisical_container_vmid",
                "canonical_path": "resources.guests.infisical.identity.vmid",
            },
            report["matrix_coverage"]["matched"],
        )
        self.assertIn(
            {
                "source": "scaffold/terraform.tfvars",
                "key": "hermes_container_vmid",
                "canonical_path": "resources.guests.hermes.identity.vmid",
            },
            report["matrix_coverage"]["matched"],
        )
        self.assertIn(
            {
                "source": "scaffold/terraform.tfvars",
                "key": "technitium_container_disk_gb",
                "canonical_path": "resources.guests.technitium.storage.root.size_gb",
            },
            report["matrix_coverage"]["matched"],
        )
        for key, canonical_path in {
            ...
            self.assertIn(
                {
                    "source": "scaffold/dns-records.local.json",
                    "key": key,
                    "canonical_path": canonical_path,
                },
                report["matrix_coverage"]["matched"],
        ...
        self.assertIn(
            {
                "source": "scripts/migrate-values.py",
                "key": "FORGEJO_UPSTREAM",
                "disposition": "retired-input",
            },
        ...
        self.assertIn(
            {
                "source": "scaffold/terraform.tfvars",
                "key": "onramp_host_vmid",
                "canonical_path": "resources.shared_hosts.onramp_host.identity.vmid",
            },
            report["matrix_coverage"]["matched"],
        )
        self.assertIn(
            {
                "source": "scaffold/terraform.tfvars",
                "key": "forgejo_runner_vmid",
                "canonical_path": "resources.guests.forgejo_runner.identity.vmid",
            },
            report["matrix_coverage"]["matched"],
        )
        self.assertIn(
            {
                "source": "scaffold/terraform.tfvars",
                "key": "tailscale_client_vmid",
                "canonical_path": "resources.guests.tailscale_client.identity.vmid",
            },
            report["matrix_coverage"]["matched"],
        )
        for key, canonical_path in (
            (
                "forgejo_runner_started",
                "resources.guests.forgejo_runner.runtime.started",
            ),
            (
                "forgejo_runner_start_on_boot",
                "resources.guests.forgejo_runner.runtime.start_on_boot",
            ),
            ("infisical_started", "resources.guests.infisical.runtime.started"),
            (
                "infisical_start_on_boot",
                "resources.guests.infisical.runtime.start_on_boot",
            ),
            ("hermes_started", "resources.guests.hermes.runtime.started"),
            ("hermes_start_on_boot", "resources.guests.hermes.runtime.start_on_boot"),
            (
                "tailscale_client_started",
                "resources.guests.tailscale_client.runtime.started",
            ),
            (
                "tailscale_client_start_on_boot",
                "resources.guests.tailscale_client.runtime.start_on_boot",
            ),
            (
                "onramp_host_started",
                "resources.shared_hosts.onramp_host.runtime.started",
            ),
            (
                "onramp_host_start_on_boot",
                "resources.shared_hosts.onramp_host.runtime.start_on_boot",
            ),
        ):
        ...
        self.assertIn(
            {
                "source": "scaffold/terraform.tfvars",
                "key": "searxng_container_image",
                "canonical_path": "services.searxng_onramp.release",
            },
            report["matrix_coverage"]["matched"],
        )
        self.assertIn(
        ...
        for source, key, canonical_path in (
            ...
            (
                "scaffold/ansible/inventory/local.yml",
                "forgejo_ssh_port",
                "services.forgejo.endpoints.ports.ssh",
            ),
            (
                "scripts/migrate-values.py",
                "FORGEJO_SSH_PORT",
        ...
        for key in (
            ...
            self.assertNotIn(
                ...
                [
                    ...
                    for x in report["matrix_coverage"]["unmatched"]
                ],
            )
        for key, canonical_path in (
        ...
        for key, canonical_path in (
            (
                ...
                "resources.shared_hosts.onramp_host.security.password_authentication",
            ),
            (
                "onramp_host_permit_root_login",
                "resources.shared_hosts.onramp_host.security.permit_root_login",
            ),
            (
                "onramp_host_deploy_user",
                "resources.shared_hosts.onramp_host.security.deploy_user",
            ),
            (
                "onramp_host_deploy_dir",
                "resources.shared_hosts.onramp_host.security.deploy_dir",
            ),
            (
                "onramp_host_allow_passwordless_sudo",
                "resources.shared_hosts.onramp_host.security.allow_passwordless_sudo",
            ),
            (
                "onramp_host_allowed_ssh_cidrs",
                "resources.shared_hosts.onramp_host.security.allowed_ssh_cidrs",
            ),
        ):
            self.assertIn(
                {
        ...
        for key, canonical_path in (
            ...
            (
                "guest_vm_image_checksum_algorithm",
                "platform.images.vm.guest.checksum.algorithm",
            ),
            ("guest_vm_image_checksum", "platform.images.vm.guest.checksum.value"),
            (
                "onramp_host_image_datastore_id",
                "platform.images.vm.onramp_host.datastore_id",
            ),
            ...
            (
                ...
                "platform.images.vm.onramp_host.checksum.algorithm",
            ),
            (
                "onramp_host_image_checksum",
                "platform.images.vm.onramp_host.checksum.value",
            ),
        ):
            self.assertIn(
                {
        ...
        self.assertIn(
            {
                "source": source,
                "key": "tailscale_client_enabled",
                "canonical_path": "services.tailscale_client.enabled",
            },
            report["matrix_coverage"]["matched"],
        )
        self.assertIn(
        ...
        self.assertEqual(len(report["service_contracts"]), 10)
        self.assertFalse(report["consumer_contract"]["legacy_terraform_input_present"])
        self.assertFalse(report["consumer_contract"]["legacy_static_inventory_present"])
        self.assertEqual(report["classification"]["inventory_status"], "complete")
        self.assertEqual(
            report["classification"]["semantic_mapping_status"],
            "semantic-coverage-complete",
        )
        self.assertEqual(
            report["classification"]["consumer_cutover_status"],
        ...
        self.assertIn(
            ...
            report["candidate_generation"]["reasons"],
        )
        self.assertEqual(
            report["candidate_projection"],
            {
                "status": "blocked",
                "row_count": 0,
        ...
        provider_aliases = report["legacy_alias_classification"][
    ...
    def test_consumer_cutover_is_canonical_only_for_selected_sites(self) -> None:
        report = MODULE.build_report(ROOT)
        self.assertEqual(
            report["classification"]["semantic_mapping_status"],
            "semantic-coverage-complete",
        ...
        self.assertEqual(
            report["consumer_contract"]["cutover_status"],
            "canonical-site-authoritative",
        )
        self.assertFalse(report["consumer_contract"]["legacy_terraform_input_present"])
        self.assertFalse(report["consumer_contract"]["legacy_static_inventory_present"])
        self.assertTrue(
            report["consumer_contract"]["canonical_projection_authoritative"]
        )
        self.assertEqual(
            report["consumer_contract"]["legacy_compatibility_scope"],
        ...
        boundary = plan + "\\n" + apply
        self.assertNotIn("terraform.tfvars", boundary)
        self.assertNotIn("ansible/inventory/local.yml", boundary)
        self.assertNotIn("plan_equivalence.py", boundary)

    def test_matrix_classification_gate_rejects_unknown_and_incoherent_rows(
        self,
    ) -> None:
        coverage = MODULE.matrix_classification_coverage(
            {
                "rows": [
                    {
                        "Canonical path": "services.example.value",
                        "Class": "new-class",
                        "Secret class": "public",
                    },
                    {
                        "Canonical path": "secrets.example.value",
                        "Class": "canonical/derived",
                        "Secret class": "secret",
                    },
                    {
                        "Canonical path": "services.example.protected",
                        "Class": "protected",
                        "Secret class": "public",
                    },
                ]
        ...
        self.assertEqual(
            ...
            [
                {
                    "canonical_path": "services.example.value",
                    "reasons": ["unknown matrix class"],
                },
                {
                    "canonical_path": "secrets.example.value",
                    "reasons": ["secret path lacks secret/protected row class"],
                },
                {
                    "canonical_path": "services.example.protected",
                    "reasons": ["secret/protected row is marked public"],
                },
            ],
        )

    def test_source_reconciliation_gate_fails_closed_for_missing_and_unexpected_inputs(
        self,
    ) -> None:
        source_inputs = {
            "inputs": [
        ...
        coverage = {
            "matched": [
                {
                    "source": "fixture.tfvars",
                    "key": "present",
                    "canonical_path": "platform.value",
                }
            ],
            "excluded": [
                {
        ...
        self.assertEqual(
            gate["missing"], [{"source": "fixture.tfvars", "key": "missing"}]
        )
        self.assertEqual(
            gate["unexpected"], [{"source": "other.tfvars", "key": "unexpected"}]
        )
        self.assertEqual(
            gate["reasons"],
            [
                "source identity lacks a matrix disposition",
                "matrix disposition references an unknown source identity",
            ],
        )
    ...
    def test_ambiguous_matrix_match_is_review_required(self) -> None:
        ...
        matrix = {
            "rows": [
                {
                    "Legacy source(s)": "`shared_key`",
                    "Canonical path": "resources.one.value",
                },
                {
                    "Legacy source(s)": "`shared_key`",
                    "Canonical path": "resources.two.value",
                },
            ]
        }
        ...
        self.assertEqual(
            coverage["unmatched"], [{"source": "fixture.tfvars", "key": "shared_key"}]
        )
        self.assertEqual(coverage["ambiguous_count"], 1)
        self.assertEqual(
            coverage["ambiguous"][0]["canonical_paths"],
            ["resources.one.value", "resources.two.value"],
        )
        self.assertEqual(coverage["status"], "review-required")
        ...
        self.assertEqual(report["opentofu"]["variable_count"], 204)
        blockers = (ROOT / "docs" / "canonical-values-model-blockers.md").read_text(
            encoding="utf-8"
        )
        for heading in (
    ...
    def test_hermes_non_secret_inputs_have_canonical_matrix_rows(self) -> None:
        ...
        self.assertEqual(
            [
                ...
                if item["key"] == "HERMES_DASHBOARD_BASIC_AUTH_PASSWORD"
            ],
            [],
        )
