...
class HermesRuntimeContractTests(unittest.TestCase):
    ...
    def test_managed_runtime_uses_hashed_wheels_and_rejects_legacy_fallback(self) -> None:
        ...
        self.assertNotIn("hermes_allow_legacy_runtime", self.main)
    ...
    def test_operator_plugin_is_installed_enabled_and_approval_gated(self) -> None:
        ...
        self.assertIn("HERMES_OPERATOR_MUTATION_ENABLED", self.gateway_unit)
        self.assertIn("hermes_operator_mutation_enabled: false", self.defaults)
        self.assertIn("hermes_operator_audit_path", self.main)
        self.assertIn("hermes_operator_audit_backup_dir", self.main)
        self.assertIn("HERMES_OPERATOR_REPO_PATH", ENV.read_text(encoding="utf-8"))
        self.assertIn("HERMES_OPERATOR_MUTATION_ENABLED", ENV.read_text(encoding="utf-8"))
